...
CRITICAL: You MUST respond ONLY in valid JSON list format. Do NOT include any explanatory text, comments, or additional information outside the JSON structure. If the entity type does not exist, return an empty JSON list: [].

Please respond in JSON list format, where each entity is represented as a dictionary. If there are no entities of a given type, return an empty list.
...
- "Synonyms": All synonyms of the entity based on knowledge (must be a JSON array)
...
[
    {
        ...
        "Domain Ontology": "hierarchy -> chain",
...
重要：必须仅以有效的JSON列表格式响应。不要包含任何解释性文字、注释或JSON结构之外的其他信息。如果实体类型不存在，请返回空JSON列表：[]。

结果请以JSON列表格式响应，其中每个实体以一个字典表示。如果在给定类型中没有实体，则返回空列表。
...
- "解释"：结合上下文及你的自身知识，对每个实体生成一段描述性的文本，避免使用代词
- "标准名"：文中的实体名可能是俗称、缩写，结合你所知的知识，给实体一个标准名称，该名称应当尽可能和其他同名实体消除歧义
- "同义词"：根据你的知识列出实体的所有同义词（必须是JSON数组）
